  ...
  myPort.onMessage.addListener((msg) => {
    ...
    if (type === 'init') {
      ...
      if (name === 'ask-send') {
        ...
        const bsvAmount = (rest.satoshis / 100000000)
          .toFixed(8)
          .replace(/(\.\d+?)0*$/, "$1");

        span.innerText = bsvAmount
      }
      ...
      if (name === 'ask-run') {
        if (rest.metadata && rest.metadata.name) {
          el.innerHTML = `
                      ...
                      <div
                        style="font-size: 14px; line-height: 16px; margin-top: 14px">
                      <div>
                      ...
                      >
                        <li>Move any amount of your tokens</li>
                      </ul>
                    </div>
